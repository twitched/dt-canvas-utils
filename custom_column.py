...
def main():
    ...
    if(args.list):
        ...
        new_column(canvas, args.course, args.new, args.file, content_column)
    elif(args.delete):
        delete_column(canvas, args.course, args.delete)
    elif(args.update):
        update_column(canvas, args.course, args.update, args.file, content_column)
...
def getargparser():
    ...
    p.add_argument('-f', '--file', help='a csv file containing the column data for --new and --update.  It should have two columns: id, content')
    p.add_argument('-o', '--content_column', help='change the "content" column to the one specified')
    return p
...
def new_column(canvas: Canvas, course_id: str, title: str, file: str, content_column: str):
    course = canvas.get_course(course_id)
    course.create_custom_column(column={'title':title, 'read_only': 'true'})
    if(file):
        update_column(canvas, course.id, title, file, content_column)
...
def update_column(canvas: Canvas, course_id: str, title: str, file: str, content_column: str):
    course = canvas.get_course(course_id)
    column = find_column_by_name(canvas, course, title)
    if(column):
        with open(file) as csvfile:
            ...
            bulk_column_data = []
            for row in reader:
                row_data = {'column_id': column.id, 'user_id': row['id'], 'content': row[content_column]}
